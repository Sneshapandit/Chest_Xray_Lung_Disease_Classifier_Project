...
# Keep track of processed and skipped images
processed_count = 0
skipped_count = 0
...
for disease in diseases:
    ...
    if os.path.isdir(class_dir):
        ...
        for img_name in os.listdir(class_dir):
            ...
            try:
                # First load the image without resizing to check its dimensions
                original_img = tf.keras.preprocessing.image.load_img(img_path, color_mode='rgb')
                original_array = tf.keras.preprocessing.image.img_to_array(original_img)
                
                # Check if the image already has the required dimensions
                if original_array.shape == (224, 224, 3):
                    img = original_array
                    images.append(img)
                    ...
                    print(f"Skipping {img_name}: Dimensions {original_array.shape} do not match required (224, 224, 3)")
                    skipped_count += 1
                    continue
...
print("Classifier model saved as classifier_model.keras")